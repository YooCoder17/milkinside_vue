  ...
  <div class="c-services__wrapper">
    <div class="c-services__sections">
      ...
      <div
        ...
        style="width:100%; padding: 0 var(--padding) 7vh;"
      >
          <transition
            name="intro_fade"
            mode="out-in"
          ...
          <transition
            ...
            mode="out-in"
          >
            <AboutSection3Footer v-if="activeStatus" style="width:64%"/>
          </transition>
          <transition
            name="image_fade"
            mode="out-in"
          >
            <video v-if="activeStatus" width="100%" id="background_video" autoplay loop muted>
              <source src="@/assets/videos/about_section3.mp4" type="video/mp4" />
...
import AboutSection3Footer from '@/components/AboutView/AboutSection3/AboutSection3Footer.vue';
import AboutSection3Heading from '@/components/AboutView/AboutSection3/AboutSection3Heading.vue';
import AboutSection3Values from '@/components/AboutView/AboutSection3/AboutSection3Values.vue';
...
methods: {
  updateLeft(){
    // this.columnWidth = (window.innerWidth - 130 * ( 0.8 / window.devicePixelRatio )) *0.7 * 0.25;
    // this.videoWidth =  (window.innerWidth - 130 )* 0.7 * 0.25 * 0.95;
    this.columnWidth = (window.innerWidth - 130) * 0.7 / 4;
    if(this.columnWidth < 320)
      this.columnWidth = 320;
    this.videoWidth = this.columnWidth * 0.9;
    this.leftMargin = window.innerWidth * 0.02;
    if(this.leftMargin < 30)
      this.leftMargin = 30;
  },
  enterIntroContent,
  leaveIntroContent,
...
.image_fade-enter-active { opacity: 1; transition: all 1.1s cubic-bezier(.71,.17,.14,.93);}
